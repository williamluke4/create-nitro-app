...
module.exports = async function install (opts) {
  ...
  const installDevArgs = getInstallArgs(installCmd, devPackages, true)
...
async function installFlowTypes(projectName){
  const stopInstallSpinner = output.wait(`Installing Flow Types`)
  try {
    await execa('flow-typed',['install'])
    stopInstallSpinner();
    output.success(`Installed flow types for ${projectName}`)
  return null;
  } catch (err) {
    stopInstallSpinner()
    output.error('Error Installing Flow Types')
    output.error(err)
    return new Error(`Flow Types installation failed`)
}

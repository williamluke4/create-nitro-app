...
For example:
  ...
  ${chalk.cyan(program.name)} ${chalk.green('my-nitro-app')}
...
exports.installing = function (packages, dev = false) {
  const pkgText = packages.map(function (pkg) {
    return `    ${chalk.cyan(chalk.bold(pkg))}`
  }).join('\n')
  return `
  Installing ${dev? 'dev': 'npm'} modules:
${pkgText}
`
}
...
exports.start = function (projectName) {
  ...
  ${chalk.green('https://github.com/williamluke4/create-nitro-app/issues')}
`
}
